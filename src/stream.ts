...
function resolvePlamoApiKey(model: RuntimeModel, options: RuntimeOptions): string | undefined {
  ...
  if (explicitApiKey) {
    return isLocalNonSecretApiKeyMarker(explicitApiKey) ? undefined : explicitApiKey;
  }
  return resolveEnvApiKey(model.provider)?.apiKey || undefined;
}

function isLocalNonSecretApiKeyMarker(value: string): boolean {
  const trimmed = value.trim();
  return (
    trimmed === PLAMO_REQUEST_AUTH_MARKER ||
    trimmed === "PLAMO_API_KEY" ||
    trimmed === "secretref-managed" ||
    trimmed.startsWith("oauth:") ||
    trimmed.startsWith("secretref-env:")
  );
}
